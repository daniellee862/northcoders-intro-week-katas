...
function readTrafficLight(lightColour) {
  ...
  const lowerCase = lightColour.toLowerCase();

  if (lowerCase === "green") {
    return "GO!";
  } else if (lowerCase === "amber") {
    return "GET READY...";
  } else {
    return "STOP!";
  }
...
function checkInfinitive(word) {
  ...
  //declare variable and assign value of last two letters.
  // return true if matches re OR ir OR er

  const lastTwoChar = word.slice(-2);

  return lastTwoChar === "re" || lastTwoChar === "ir" || lastTwoChar === "er" ? true : false;
}
...
function getOrdinalSuffix(num) {
  ...
  //I can get this to past the tests but if you was to pass in a number over 100 im not immediately sure how to do that.
  //update....just check last digit?
  //update update the most 'parental' outter parts of the if statment will check if the number end in the 'teens'
  const numString = String(num);
  const lastString = String(num).slice(-1);
  const lastNumber = Number(lastString);
  const last2String = String(num).slice(-2);
  const last2Number = Number(last2String);

  if (numString.length > 1 && last2Number >= 10) {
  }
