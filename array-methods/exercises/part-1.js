...
runTest("can get the names of staff over 28", function () {
  ...
  const namesOfStaffOver28 = staff.filter((person) => person.age > 28).map((person) => person.name);
...
runTest("can find the index position of X", function () {
  ...
  const treasureArray = treasureMap.split("");
  const positionOfX = treasureArray.indexOf("X");
