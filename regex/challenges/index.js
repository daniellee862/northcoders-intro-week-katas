...
Try this link -> https://javascript.info/regexp-introduction#regular /\b[abc]{6}/ /\b[abc]{6}/ /\b[abc]{6}/-expressions
*/
...
const extractCode = (param) => {
  return Number(param.match(/[0-9]+/));
};
...
const isValidSortCode = (param) => {
  return /^\d\d-\d\d-\d\d$/.test(param);
};
...
const isProfessionalEmail = (email) => {
  return /x$/i.test(email) === true ? false : true;
};
...
const testValidCountdown = (str) => {
  if (str.length === 9) {
    return /[aeiou]{3}[bcdfghjklmnpqrstvwxyz]{4}/.test(str);
  }
